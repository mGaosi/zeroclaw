...
- **flutter_rust_bridge (FRB)** is an external code generation tool that consumes ZeroClaw's `src/api/` types. It does not appear as a Cargo dependency in ZeroClaw itself. FRB compatibility is validated by ensuring all public API types use concrete, `pub`, FRB-translatable signatures (T053).
