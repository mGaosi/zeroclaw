...
async fn host_tool_full_round_trip() {
    ...
    conversation::send_message(&handle, "say hello to the world".into(), tx).unwrap();
...
async fn dynamic_registration_round_trip() {
    ...
    conversation::send_message(&handle, "use alpha".into(), tx1).unwrap();
    ...
    conversation::send_message(&handle, "use beta".into(), tx2).unwrap();
...
async fn streaming_events_order_for_host_tool() {
    ...
    let (tx, rx) = mpsc::channel::<StreamEvent>(64);
    conversation::send_message(&handle, "trigger stream_tool".into(), tx).unwrap();
...
async fn config_rebuild_preserves_host_tools() {
    ...
    // First use
    let (tx1, rx1) = mpsc::channel::<StreamEvent>(64);
    conversation::send_message(&handle, "trigger rebuild_tool".into(), tx1).unwrap();
    ...
    let (tx2, rx2) = mpsc::channel::<StreamEvent>(64);
    conversation::send_message(&handle, "trigger rebuild_tool again".into(), tx2).unwrap();
