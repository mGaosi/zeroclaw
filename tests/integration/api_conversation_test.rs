...
async fn send_message_text_roundtrip() {
    ...
    conversation::send_message(&handle, "hi".into(), tx).unwrap();
...
async fn send_message_empty_message_errors() {
    ...
    let result = conversation::send_message(&handle, String::new(), tx);
...
async fn send_message_with_tool_calls() {
    ...
    let (tx, rx) = tokio::sync::mpsc::channel(32);
    conversation::send_message(&handle, "use a tool".into(), tx).unwrap();
